 ...
 * 
 * @return Zero when success, otherwise return -1
 *
 */
PUBLIC int do_exec()
{
	...
	phys_copy((void*)va2la(TASK_MM, pathname), (void*)va2la(src, mm_msg.PATHNAME), \
		name_len);
	...
	for(i = 0; i < elf_hdr->e_phnum; i++) {
		Elf32_Phdr* prog_hdr = (Elf32_Phdr*)(mmbuf + elf_hdr->e_phoff + \
			i * elf_hdr->e_phentsize);	// todo check
		if(prog_hdr->p_type == PT_LOAD) {
			assert(prog_hdr->p_vaddr + prog_hdr->p_memsz < PROC_IMAGE_SIZE_DEFAULT);
			phys_copy((void*)va2la(src, (void*)prog_hdr->p_vaddr), (void*)va2la(TASK_MM, \
				mmbuf + prog_hdr->p_offset), prog_hdr->p_filesz);
		}
	}
	...
	phys_copy((void*)va2la(TASK_MM, stackcopy), (void*)va2la(src, mm_msg.BUF), \
		orig_stack_len);
	...
	int argc = 0;
	if(orig_stack_len) {	/* if has args */
		char **q = (char**)stackcopy;
		for(; *q != 0; q++, argc++)
			*q += delta;
	}

	phys_copy((void*)va2la(src, orig_stack), (void*)va2la(TASK_MM, stackcopy), \
		orig_stack_len);
