...
DispMemInfo:
	...
	mov esi, MemChkBuf
	mov ecx, [dwMCRNumber]		; for(int i = 0; i < [MCRNumber]; i++)  // 每次得到一个 ARDS（Address Range Descriptor Structure）结构
.loop:
	mov edx, 5					; for(int j = 0; j < 5; j++) // 每次得到一个 ARDS 中的成员，共5个成员
...
SetupPaging:
	...
	test edx, edx
	jz .no_remainder
	inc eax 			; 如果余数不为0，就需要增加一个页表
.no_remainder:
	push ecx			; 暂存页表个数
	
	; 为简化处理，所有线性地址对应相等的物理地址，并且不考虑内存碎片的问题。
	; 首先初始化页目录
	mov ax, SelectorFlatRW
	mov es, ax
...
; ------------------------------------------------------------------------
; 函数名：InitKernel()
; 作用：将 KERNEL.BIN 的内容经过整理对齐后放到新的位置
; ------------------------------------------------------------------------
InitKernel:			; 遍历每一个 Program Header, 根据 Program Header 中的信息来确定把什么放进内存，放到什么位置，以及放多少进去
	xor esi, esi
...
.Begin:
	...
	mov eax, [esi + 04h]			;      ┃
	add eax, KERNEL_FILE_PHY_ADDR	; 	   ┣ ::memcpy((void*)(pPHdr->p_vaddr), uchCode + pPHdr->p_offset, pPHdr->p_filesz);
	push eax 						; src  ┃
	push dword [esi + 08h]			;      ┃
	call MemCpy 					;      ┃
	add esp, 12						;      ┛
...
_dwDispPos: dd (80 * 7 + 0) * 2 	; 屏幕第7行第0列
_dwMemSize: dd 0
