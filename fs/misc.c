 ...
 * such as open(), read() and write(). It accepts the full path and returns two things: the basename and a ptr of the root dir's i-node.
 *
 ...
 *    - ret val: 0 (success)
 *
 * Current an acceptable pathname should begin with at most one `/` preceding a filename.
 *
 ...
 * @return Zero if success, -1 when pathname is not valid.
 *
 */
